...
- Prototype Python đã được viết và có thể chạy tự kiểm tra logic offline. Sau khi giảng viên xác nhận **được dùng ChatGPT/OpenAI cho bài lab**, tôi dùng `OPENAI_API_KEY` sẵn có để chạy `gpt-4.1-mini`: cả 3 input tấn công đều qua validator; phản hồi và trạng thái có trong [openai-sanity-results.json](openai-sanity-results.json). Sau khi sửa lỗi mã hóa tiếng Việt trên Windows, lệnh `python starter-code/prompt_prototype.py` chạy thành công và `python autograder/autograder.py` đạt **10/10 trên máy**. Kết quả này là của **OpenAI**, không được ghi thành lượt kiểm thử Gemini. Khi có Gemini key, cùng code vẫn hỗ trợ chạy Gemini để so sánh.
